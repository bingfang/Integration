...
open IN, "<Clusters2blat.fa" or die;
while ($line=<IN>) {
	chomp($line);
	if ($line=~/^>(.*)/) {
		$id=$1;
...
open IN, "<Clusters2blat_blat.txt" or die;
open OUT, ">Clusters2blat_blat.filter1.txt" or die;
open OUTS, ">Clusters2blat_blat.summary.txt" or die;
