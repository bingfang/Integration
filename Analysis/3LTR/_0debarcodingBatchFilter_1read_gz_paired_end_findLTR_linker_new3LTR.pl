  ...
  # at the end of the scope, $tmp vanishes 
	...
	while ($lineread1=<INread1>) {
		...
		if($lineread1=~/(^@.*)( 1.*)/) {
			...
			if($read2_seq=~/......AGGGCTCCGCTTAAGGGACT(.*)/) {  
				$read2_seq_trimmed=$1;
			...
			$read1_nestedPrimer=substr($read1_seq,6,24);
			...
			if ($read1_nestedPrimer eq "CCCTTTTAGTCAGTGTGGAAAATC") { 
				($preLTRend, $postLTRend) = split (/TCTAGCA/, $LTR_junction);
				$LTR_junction= "$preLTRend"."TCTCTAGCA\t"."$postLTRend";
			}
